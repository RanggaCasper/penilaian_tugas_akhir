...
@section('content')
    <x-card title="Tambah Jadwal Ujian">
        <form action="{{ route('admin.proposal.schedule.store') }}" method="POST">
            @csrf
            <div class="row">
                <div class="col-lg-6">
                    <div class="mb-3">
                ...
                <div class="col-lg-6">
                    <div class="mb-3">
                        ...
                        <select id="student" name="student_id" class="form-control select2" 
                                ...
                                data-ajax-url="{{ route('admin.proposal.schedule.getStudent') }}" 
                    ...
                    <div class="mb-3">
                        ...
                        <select id="examiner-1" name="primary_examiner_id" class="form-control select2" 
                                ...
                                data-ajax-url="{{ route('admin.proposal.schedule.getExaminer') }}" 
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" id="form_update">
                        ...
                        @method("PUT")
                        <div class="mb-3">
                            <x-input-field label="Tanggal Ujian" type="date" name="exam_date" id="exam_date_update" />
                        </div>
                        <div class="mb-3">
...
<script>
    ...
    function downloadFile(format) {
        ...
        if (examDate) {
            ...
            $.ajax({
                url: '{{ route("admin.proposal.schedule.get") }}',
                type: 'GET',
                data: {
                    export: format,
                    exam_date: examDate
    ...
    $('#datatables').DataTable({
        ...
        serverSide: false,
        scrollX: true,
        ajax: '{{ route('admin.proposal.schedule.get') }}',
        columns: [
            { data: 'no', name: 'no' },
            ...
            { data: 'student.proposal.title', name: 'student.proposal.title' },
    ...
    $('#datatables').on('click', '.edit-btn', function() {
        var id = $(this).data('id');
        $.ajax({
            url: '{{ route("admin.proposal.schedule.getById", ["id" => ":id"]) }}'.replace(':id', id),
            type: 'GET',
            success: function(data) {
                $('#form_update').attr('action', '{{ route("admin.proposal.schedule.update", ["id" => ":id"]) }}'.replace(':id', id));
    ...
    $('#datatables').on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if(t.value) {
                $.ajax({
                    url: '{{ route("admin.proposal.schedule.destroy", ["id" => ":id"]) }}'.replace(':id', id),
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        _method: 'DELETE'
