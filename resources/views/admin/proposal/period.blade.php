...
<script>
    ...
    $('#datatables').on('click', '.edit-btn', function() {
        var id = $(this).data('id');
        $.ajax({
            url: '{{ route("admin.periode.proposal.getById", ["id" => ":id"]) }}'.replace(':id', id),
            type: 'GET',
            success: function(data) {
                $('#form_update').attr('action', '{{ route("admin.periode.proposal.update", ["id" => ":id"]) }}'.replace(':id', id));
    ...
    $('#datatables').on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if(t.value) {
                $.ajax({
                    url: '{{ route("admin.periode.proposal.destroy", ["id" => ":id"]) }}'.replace(':id', id),
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        _method: 'DELETE'
